...
// ******************************************************
    ...
    if( ( inputBuffer = malloc(8192) ) == NULL )
    {
    ...
    if( ( outputBuffer = malloc(8192) ) == NULL )
    {
...
//
	...
	t_start = get_timestamp();
	int amount_read = snd_pcm_readi(pcm_capture_handle, inputBuffer, exact_bufsize);
...
//	audio_process((short *)outputBuffer, (short *)inputBuffer, blksize/2);
	...
	errcnt = 0;	// The Beagle gets an underrun error the first time it trys to write,
        ...
        int amount_written = snd_pcm_writei(pcm_output_handle, whichBuffer, exact_bufsize);
        //printf("amount_written: %d\n", amount_written);
	while (amount_written < 0) {
	    snd_pcm_prepare(pcm_output_handle);
	    ERR( "<<<<<<<<<<<<<<< Buffer Underrun >>>>>>>>>>>>>>> err=%d, errcnt=%d\n", err, errcnt);
	    //memset(whichBuffer, 0, blksize);		// Clear the buffer
	    amount_written = snd_pcm_writei(pcm_output_handle, whichBuffer, exact_bufsize);
            printf("amount_written again: %d\n", amount_written);
	}
	//printf("Jack says Hi~~~\n");
